...
class Settings(BaseSettings):
    ...
    fb_locations: str = Field(
        ...
        description="Comma-separated name:fb_id pairs for FB Marketplace locations",
    )
    ...
    def fb_locations_map(self) -> dict[str, str]:
        """Return {name: fb_marketplace_id} from 'name:id,name:id' format."""
        result = {}
        for item in self.fb_locations.split(","):
            item = item.strip()
            if ":" in item:
                name, fb_id = item.split(":", 1)
                result[name.strip()] = fb_id.strip()
            else:
                result[item] = item  # fallback
        return result
